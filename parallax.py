#!/usr/bin/env python3
"""
parallax.py — AlgoTrade 2026 trading bot.

Concept
-------
Every quote on every venue is a noisy, latency-delayed observation of one
common fair value.  PARALLAX views each instrument from many "viewing angles"
at once — its book on each listing venue, the basket NAVs that contain it,
the sector cohort it belongs to, and the inverse safe-haven index — and
triangulates a consensus fair value.  It then trades stale quotes wherever
they appear, hardest where the triangulation has the most agreement.
...
Run
---
    python3 parallax.py                       # all 10 venues
    python3 parallax.py --venues ZSE,NYSE     # subset
    LOGLEVEL=DEBUG python3 parallax.py        # verbose tracing
"""
...
class Hub:
    ...
    def etf_basket_arbs(self) -> list[Plan]:
        ...
        for etf in ETFS:
            ...
            # For each constituent, find best buy venue and best sell venue.
            ...
            for etf_v in LISTINGS[etf]:
                ...
                # Direction A: ETF cheap → BUY ETF, SELL constituents on best-bid venues.
                etf_buy_px = ba
                etf_buy_qty_avail = bk.best_ask_qty
                synth_sell_per_unit = sum(const_bid[tk][1] for tk in basket) / n
                edge_a = synth_sell_per_unit - etf_buy_px
                if edge_a >= edge_floor:
                    k = etf_buy_qty_avail // n
                    for tk in basket:
                ...
                # Direction B: ETF rich → SELL ETF, BUY constituents on best-ask venues.
                etf_sell_px = bb
                etf_sell_qty_avail = bk.best_bid_qty
                synth_buy_per_unit = sum(const_ask[tk][1] for tk in basket) / n
                edge_b = etf_sell_px - synth_buy_per_unit
                if edge_b >= edge_floor:
                    k = etf_sell_qty_avail // n
                    for tk in basket:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="parallax — AlgoTrade 2026 bot")
    p.add_argument(
        "--venues",
        default=",".join(VENUES),
...
def main() -> None:
    ...
    log.info("starting parallax on venues: %s", active)
    try:
