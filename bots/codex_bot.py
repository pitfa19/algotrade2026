#!/usr/bin/env python3
"""
AlgoTrade 2026 — improved bot.

Built on the provided sample bot. Keeps the same async/websocket infrastructure,
risk manager pattern, and dry-run-by-default ergonomics, but adds:

  1. Microprice-based fair value (depth-weighted mid, more accurate than naive mid).
  2. Leader-weighted multi-exchange fair value (primary venues weighted higher).
  3. Latency-arbitrage strategy: when a fast cluster moves and the slow cluster
     hasn't caught up, fire IOC on the slow cluster in the direction of the move.
  4. Per-instrument edge thresholds, calibrated from observed spreads.
  5. Edge-scaled order sizing, capped by book depth and risk.
  6. Aggressor flow tracking — lets directional pressure bias thresholds.
  7. Rolling mid history per (exchange, ticker) for lead-lag detection.
...
class MarketState:
    def __init__(self, history_len: int = 8) -> None:
        ...
        # Aggressor flow per ticker: +1 per buy aggressor, -1 per sell aggressor
    ...
    def apply_market_data(
        ...
        for event in message.get("events", []):
            ...
            flow_q = self._flow.setdefault(ticker, deque(maxlen=20))
...
class StrategyEngine:
    ...
    def _build_opp(
        ...
        rtt = LATENCY_RTT_MS.get(snapshot.exchange, {}).get(snapshot.exchange, 0)  # local roundtrip ~0
...
class ExchangeClient:
    ...
    def on_add_order_response(self, msg: dict[str, Any]) -> None:
        ...
        if not msg.get("success", False):
            ...
            return
        self.risk.apply_immediate_fill(opp, msg.get("data", {}))
