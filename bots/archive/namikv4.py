#!/usr/bin/env python3
"""
namikv4.py — AlgoTrade 2026 trading bot, CARD/SIMP only.

Stripped-down derivative of namikv2 that trades only CARD and SIMP. Both are
listed on all 10 exchanges (the only tickers with full coverage besides what
ZSE alone carries), so they have the deepest cross-venue surface in the
universe. There are no ETF/sub-ETF strategies because neither ticker is part
of any basket — XV arb and passive MM are the only edges, plus settlement
unwind for end-of-segment hygiene.

Kept from v2:
  * Per-instrument adaptive XV thresholds (rolling time-window quantile).
  * Depth walking on book sweeps.
  * Microprice for MM skew.
  * Inventory-tapered sizing.
  * Per-strategy P&L instrumentation.
  * Cluster-aware tie-breaking + cross-cluster drift padding.
  * Optimistic cash/position commits with revert on rejection.
  * Reconnect / segment-boundary reset / round-length parsing from welcome.

Removed:
  * ETF basket arb, sub-ETF arb (no ETFs in scope).
  * All instruments other than CARD/SIMP (universe + listings table).
"""
...
# ════════════════════════════════════════════════════════════════════════════
# Universe — CARD/SIMP only, but every venue carries them
# ════════════════════════════════════════════════════════════════════════════
...
TICKERS = ["CARD", "SIMP"]
LISTINGS: dict[str, set[str]] = {tk: set(VENUES) for tk in TICKERS}
...
class Hub:
    ...
    def req_id(self, tag: str) -> str:
        self._req_seq += 1
        return f"n{self._req_seq:08x}-{tag}"
    ...
    def _send_leg(self, leg: Leg) -> bool:
        ...
        if conn is None or not conn.connected:
            return False
        rid = self.req_id(leg.note[:16] if leg.note else leg.ticker)
    ...
    def on_md(self, exchange: str, msg: dict) -> None:
        ...
        for inst, depth in msg.get("orderbook_depths", {}).items():
            ...
            if ex != exchange or tk not in LISTINGS:
                continue
    ...
    def on_inventory(self, exchange: str, msg: dict) -> None:
        ...
        for inst, pair in data.items():
            ...
            if inst == "$":
                ...
                ex, _, tk = inst.partition("-")
                if ex == exchange and tk in LISTINGS:
    ...
    def on_pending_orders(self, exchange: str, msg: dict) -> None:
        ...
        for inst, pair in data.items():
            ex, _, tk = inst.partition("-")
            if ex != exchange or tk not in LISTINGS or not isinstance(pair, list) or len(pair) < 2:
                continue
    ...
    def strategize(self) -> None:
        for plan in self.cross_venue_arbs():
            self.fire(plan)
        for plan in self.settlement_unwind():
            self.fire(plan)
    ...
    def cross_venue_arbs(self) -> list[Plan]:
        ...
        for tk in TICKERS:
            ...
            qty = max(0, int(qty * min(taper_buy, taper_sell)))
    ...
    # ─── strategy 2: settlement-aware unwind ─────────────────────────
    def settlement_unwind(self) -> list[Plan]:
        ...
        for ex in self.active_venues:
            ...
            for (ex2, tk), pos in list(self.pos.items()):
                if ex2 != ex or pos == 0 or tk not in LISTINGS:
                    continue
...
def main() -> None:
    ...
    for v in args.venues.split(","):
        ...
        if canon not in active:
            active.append(canon)
    log.info("starting namikv4 (CARD/SIMP only) on venues: %s", active)
