#!/usr/bin/env python3
"""
prism.py — AlgoTrade 2026 trading bot.

Concept
-------
PRISM treats every quoted price across every venue as a noisy observation of an
underlying fair value, and every ETF quote as an observation of the *sum* of its
constituents.  It refracts those observations into one coherent fair-value tape
per instrument, then fires multi-leg IOC arbs whenever a quote on any venue
diverges from the synthetic tape by more than a configurable edge.

Edges exploited (in priority order):

  1. **Multi-venue ETF<->basket arbitrage.**  When an ETF trades away from its
     equal-weighted basket NAV, we hit it.  Each constituent leg is routed to
     its *cheapest* venue (best ask if buying, best bid if selling), not just
     the listing venue of the ETF.  Because most ETFs are missing some
     constituents on their listing venue, a same-venue-only arb is often
     impossible -- multi-venue routing is what makes the trade live at all.

  2. **Sub-ETF arbitrage.**  ETFA3's basket (NGUP, KTST, XFR) is a strict
     subset of ETFA's, so there is a closed-form linear identity:

         2 * ETFA  =  ETFA3 + (OIT + FSR + JZRO) / 3

     When ETFA3 is mispriced relative to ETFA + the 3-stock complement basket,
     we trade it directly against the synthetic.  Same idea for ETFB / ETFB3.

  3. **Cross-venue same-stock arbitrage.**  ZSE lists every stock; CARD/SIMP
     are on all 10 venues.  When a stock's best bid on one venue exceeds its
     best ask on another, we sweep both sides.

  4. **Step-inside-MM passive making.**  The built-in MM quotes 5 levels x 50
     shares with a skew driven by its own inventory.  We sit one tick inside
     its top quote on a small set of high-flow instruments and harvest noise
     traders.  Zero fees + price-time priority means a 1-tick inside quote
     captures all noise flow until the MM moves.

  5. **Settlement-aware unwind.**  Final segment marking is unspecified; we
     cut inventory in the last 60s and IOC-flatten in the last 8s.

Defensive bits:

  * Local 400 msg/s/exchange token bucket -- 80% of the 500/s server cap.
  * Per-leg position / cash headroom checks before any plan fires.  A plan
    with any infeasible leg is dropped whole, never half-executed.
  * Optimistic local position tracking with periodic get_inventory reconcile
    every 2 s as authoritative truth.
  * Reconnect loop with exponential backoff handles segment boundaries (the
    server resets state between 10-min segments).

Run
---
    python3 prism.py                       # connects to every exchange
    python3 prism.py --venues ZSE,NYSE     # subset for testing
    LOGLEVEL=DEBUG python3 prism.py        # verbose tracing
"""
...
@dataclass
class Book:
    # best_* are cached on every update() so strategy hot-paths don't re-scan
    # bids/asks dicts.  This was the single largest CPU sink: max()/min() on
    # @property accessors got called ~hundreds of times per strategize() at
    # ~110 strategize/sec.
    ...
    def update(self, depth: dict) -> None:
        bids = {int(p): int(q) for p, q in depth.get("bids", {}).items()}
        asks = {int(p): int(q) for p, q in depth.get("asks", {}).items()}
        self.bids = bids
        self.asks = asks
        if bids:
            bb = max(bids)
            self.best_bid = bb
            self.best_bid_qty = bids[bb]
        else:
            self.best_bid = None
            self.best_bid_qty = 0
        if asks:
    ...
    def reset(self) -> None:
        """Clear the book (used on (re)connect before fresh data arrives)."""
        self.bids = {}
        self.asks = {}
        self.best_bid = None
        self.best_ask = None
...
class Hub:
    def __init__(self, active_venues: list[str]):
        ...
        # O(1) membership for the hot path (`v in self.active_venues_set`).
        # The list is kept for ordered iteration where order matters.
        self.active_venues_set: set[str] = set(active_venues)
        self.stop = False
        ...
        for tk, venues in LISTINGS.items():
            for v in venues:
                if v in self.active_venues_set:
                    self.books[(v, tk)] = Book()

        # Pre-filtered active venues per ticker — saves a list-comprehension on
        # every strategy iteration.  Same ordering as LISTINGS iteration order.
        self.venues_for_ticker: dict[str, tuple[str, ...]] = {
            tk: tuple(v for v in vs if v in self.active_venues_set)
        ...
        # Index: (exchange, ticker) -> set of live order IDs.  Lets passive_mm_refresh
        # find quotes on a specific instrument in O(quotes_on_inst) instead of
        # O(all_live_orders).  Must stay in sync with live_orders -- always go
        # through _live_set / _live_pop.
        self.live_orders_by_inst: dict[tuple[str, str], set[int]] = defaultdict(set)
    ...
    # ─── live-order book-keeping (keeps index in sync) ───────────────
    def _live_set(self, oid: int, ex: str, tk: str, side: str,
                  qty: int, px: int) -> None:
        self.live_orders[oid] = (ex, tk, side, qty, px)
        self.live_orders_by_inst[(ex, tk)].add(oid)

    def _live_pop(self, oid: int) -> Optional[tuple[str, str, str, int, int]]:
        rec = self.live_orders.pop(oid, None)
        if rec is not None:
            self.live_orders_by_inst[(rec[0], rec[1])].discard(oid)
        return rec
    ...
    def on_welcome(self, exchange: str) -> None:
        ...
        for tk in self.tickers_on_ex.get(exchange, set()):
            ...
            if book is not None:
                book.reset()
            # Clear any per-instrument live-order index entries on this venue.
            self.live_orders_by_inst.pop((exchange, tk), None)
        self.cash[exchange] = INITIAL_CASH
        for oid in [oid for oid, rec in self.live_orders.items()
                    if rec[0] == exchange]:
            ...
            for ev in events:
                ...
                for oid_key in ("passiveOrderID", "activeOrderID"):
                    ...
                    if rem <= 0:
                        ...
                        # Same instrument/side/px, just reduce remaining qty.
                        live_orders[oid] = (ex, tk, side, rem, px)
    ...
    def on_add_order_response(self, exchange: str, msg: dict) -> None:
        ...
        if leg.order_type == "limit" and oid is not None:
            ...
            if remaining > 0:
                self._live_set(int(oid), leg.exchange, leg.ticker,
                               leg.side, remaining, leg.price)
        else:
            self.open_count[exchange] = max(0, self.open_count[exchange] - 1)
    ...
    def on_pending_orders(self, exchange: str, msg: dict) -> None:
        ...
        for inst, pair in data.items():
            ...
            for side_idx, side_label in enumerate(("bid", "ask")):
                for od in pair[side_idx] or []:
                    ...
                    px  = int(od["price"])
                    self._live_set(oid, ex, tk, side_label, qty, px)
                    live_ids.add(oid)
        ...
        stale = [oid for oid, rec in self.live_orders.items()
                 if rec[0] == exchange and oid not in live_ids]
        for oid in stale:
            self._live_pop(oid)
    ...
    def strategize(self) -> None:
        ...
        # Order matters: highest-confidence arbs first so they grab capacity
        # before the looser strategies.
        plans: list[Plan] = []
    ...
    def etf_basket_arbs(self) -> list[Plan]:
        out: list[Plan] = []
        books = self.books
        venues_for = self.venues_for_ticker
        for etf in ETFS:
            ...
            for etf_v in venues_for[etf]:
                ...
                # Per-share basket value when we sell each constituent at best bid:
                # (sum_of_bids) / n; one ETF unit corresponds to (1/n) of each constituent.
                # For k basket-multiples, ETF qty = k*n, each constituent qty = k.
                # P&L per k: revenue_sells - cost_etf_buy
                #          = k * sum_of_bids - k*n * etf_buy_px
                # Edge per share of ETF = (sum_of_bids/n) - etf_buy_px
                ...
                if edge_a >= ARB_EDGE:
                    ...
                    for tk in basket:
                        ...
                        k = min(k, room)
                    # Cash headroom: ETF buy = k*n*etf_buy_px on etf_v
                    cash_room = (self.cash[etf_v] - SOFT_CASH_FLOOR) // (n * etf_buy_px) if etf_buy_px > 0 else 0
                ...
                if edge_b >= ARB_EDGE:
                    ...
                    # Since cash is per-venue and constituents may span venues,
    ...
    def sub_etf_arbs(self) -> list[Plan]:
        ...
        for super_etf, (sub_etf, complement) in SUB_ETF_LINKS.items():
            sup_venues = venues_for[super_etf]
            sub_venues = venues_for[sub_etf]
            ...
            for tk in complement:
                ...
                for v in venues_for[tk]:
                    bk = books[(v, tk)]
                    bbid, bask = bk.best_bid, bk.best_ask
                    if bbid is not None and (best_b is None or bbid > best_b[1]):
                        best_b = (v, bbid, bk.best_bid_qty)
                    if bask is not None and (best_a is None or bask < best_a[1]):
                        best_a = (v, bask, bk.best_ask_qty)
                if best_b is None or best_a is None:
                    ok = False
                    break
                comp_bid[tk] = best_b
    ...
    def passive_mm_refresh(self) -> None:
        ...
        now = time.monotonic()
        active = self.active_venues_set
        ready = self.ready
        for tk in MM_INSTRUMENTS:
            for v in LISTINGS.get(tk, ()):  # set is fine for membership but we need iter
                if v not in active or not ready.get(v, False):
                    continue
                key = (v, tk)
                ...
                # Use the per-instrument index instead of scanning all live_orders.
                have_bid_at_px = False
                have_ask_at_px = False
                stale_oids: list[int] = []
                for oid in self.live_orders_by_inst.get(key, ()):
                    rec = self.live_orders.get(oid)
                    if rec is None:
                        continue
                    side0, px0 = rec[2], rec[4]
...
def main() -> None:
    ...
    log.info("starting prism on venues: %s", active)
    try:
