...
# Drop-in faster event loop. No-op fallback if not installed.
try:
    import uvloop
    uvloop.install()
except ImportError:
...
# Faster JSON. orjson.dumps returns bytes — websockets accepts bytes,
# so we don't need to decode on the send path.
try:
    ...
    def _dumps(d):
        return _json_mod.dumps(d, separators=(",", ":")).encode()
...
RATE_PER_S         =    480       # 20/s safety margin under server cap of 500
RATE_BURST         =    500       # bucket opens fully primed at server cap
SOFT_POS_MAX       =  1_800
SOFT_POS_MIN       =   -180
...
MM_INSIDE_TICK     =      1
MM_QTY             =      4
MM_REFRESH_S       =      0.5     # 3× more requotes vs v2-baseline 1.5
...
# Cold-start window. For the first COLD_START_MS of server time per venue we
# fire on ~any positive edge (1¢ floor), skip adaptive quantile and inventory
# taper, and double MM size. The leader's first-few-seconds advantage comes
# from this kind of unconstrained firing while spreads are still wide.
COLD_START_MS              = 5_000
COLD_START_EDGE_FLOOR      =     1
COLD_START_MM_QTY          =     8

# Post-fill scalper. After we take liquidity in any X-VENUE / ETF-NAV /
# SUB-ETF leg, immediately rest a limit on the opposite side at fill_price ±
# SCALP_OFFSET so the trade rotates back to cash. This is the leader's
# signature behaviour — keeps cash high, position small.
SCALP_OFFSET_CENTS         =     1
SCALP_DEADLINE_MS          =   800
SCALP_QUEUE_MAX            =   200
...
class Hub:
    def __init__(self, active_venues: list[str], my_cluster: str = "NA"):
        ...
        self.buckets: dict[str, TokenBucket] = {
            v: TokenBucket(RATE_PER_S, burst=RATE_BURST) for v in active_venues
        }
        ...
        # Cold-start window (per-venue). Populated by on_welcome().
        self.cold_start_until_ms: dict[str, int] = {v: 0 for v in active_venues}

        # Post-fill round-trip scalper queue.
        # entry: (exchange, ticker, side_filled, fill_price, fill_qty, deadline_ms)
        self.scalp_queue: deque[tuple[str, str, str, int, int, int]] = deque(
            maxlen=SCALP_QUEUE_MAX
        )
        # Live scalp orders so we can cancel past their deadline.
        # order_id -> (exchange, ticker, deadline_ms)
        self.scalp_live: dict[int, tuple[str, str, int]] = {}
    ...
    # ─── cold-start helpers ──────────────────────────────────────────
    def is_cold_start(self, exchange: str) -> bool:
        until = self.cold_start_until_ms.get(exchange, 0)
        if until == 0:
            return False
        return self.server_time.get(exchange, 0) < until

    def any_cold_start(self) -> bool:
        return any(self.is_cold_start(v) for v in self.active_venues)

    def _threshold(self, strategy: str, ticker: str, base_floor: int, *venues: str) -> int:
        """Adaptive threshold, but force the cold-start floor when any of the
        passed venues is still in its cold-start window. We do *not* feed the
        quantile during cold-start since we want to fire on near-anything."""
        if any(v and self.is_cold_start(v) for v in venues):
            return COLD_START_EDGE_FLOOR
        return self.spread_tracker.threshold(strategy, ticker, base_floor)
    ...
    def inventory_taper(self, exchange: str, ticker: str, side: str) -> float:
        ...
        During cold-start we skip the taper entirely — cash and position
        capacity are abundant, and we want to fire as much as the rate
        budget allows.
        """
        if self.is_cold_start(exchange):
            return 1.0
    ...
    def on_welcome(self, exchange: str, msg: Optional[dict] = None) -> None:
        ...
        # We *know* the post-welcome state at round start: cash=INITIAL_CASH,
        # all positions=0, no live orders. Mark ready=True now and let
        # get_inventory just verify — saves one RTT before our first fire.
        self.ready[exchange] = True
        # Cold-start window: keep aggressive thresholds for the first
        # COLD_START_MS of server time on this exchange.
        st = self.server_time.get(exchange, 0)
        self.cold_start_until_ms[exchange] = st + COLD_START_MS
    ...
    def on_md(self, exchange: str, msg: dict) -> None:
        ...
        for ev in msg.get("events", []) or []:
            ...
            for oid_key in ("passiveOrderID", "activeOrderID"):
                ...
                # Attribute the fill. SCALP resting orders look identical to
                # MM here — disambiguate by checking scalp_live.
                attrib = "SCALP" if oid in self.scalp_live else "MM"
                self.stats.record_fill(attrib, cash_delta)
                rem = qty - fill_qty
                if rem <= 0:
                    self.live_orders.pop(oid, None)
                    self.scalp_live.pop(oid, None)
                else:
    ...
    def on_add_order_response(self, exchange: str, msg: dict) -> None:
        ...
        # Round-trip scalper push: any successful arb fill (X-VENUE / ETF-NAV /
        # SUB-ETF) goes back the other way at fill_avg ± SCALP_OFFSET. We push
        # to the queue and let strategize() drain it on the next tick.
        if (ic is not None and bc is not None and leg.strategy
                and leg.strategy.split()[0] in ("X-VENUE", "ETF-NAV", "SUB-ETF")):
            fill_qty = abs(int(ic))
            if fill_qty > 0:
                avg_fill_px = abs(int(bc)) // fill_qty
                self.scalp_queue.append((
                    leg.exchange, leg.ticker, leg.side,
                    avg_fill_px, fill_qty,
                    now_ms() + SCALP_DEADLINE_MS,
                ))
        ...
        if leg.order_type == "limit" and oid is not None:
            ...
            if remaining > 0:
                ...
                # Register scalp resting orders so we can cancel them past
                # their deadline and so trade-event attribution finds them.
                if leg.strategy == "SCALP":
                    self.scalp_live[int(oid)] = (
                        leg.exchange, leg.ticker, now_ms() + SCALP_DEADLINE_MS,
                    )
        else:
            self.open_count[exchange] = max(0, self.open_count[exchange] - 1)
    ...
    def on_inventory(self, exchange: str, msg: dict) -> None:
        ...
        if not isinstance(data, dict):
            return
        # ready was already set in on_welcome (we know post-welcome state).
        # Verify the server agrees and warn if it doesn't — a real mismatch
        # means we ran with bad cached state and any orders we sent during
        # the cold-start window may have been mispriced.
        for inst, pair in data.items():
            try:
                ...
                continue
            if inst == "$":
                cached = self.cash[exchange]
                if cached != total:
                    log.warning("inventory mismatch %s cash cached=%d server=%d",
                                exchange, cached, total)
                self.cash[exchange] = total
            else:
                ex, _, tk = inst.partition("-")
                if ex == exchange:
                    cached = self.pos[(ex, tk)]
                    if cached != total:
                        log.warning("inventory mismatch %s-%s pos cached=%d server=%d",
                                    exchange, tk, cached, total)
    ...
    # ─── post-fill round-trip scalper ────────────────────────────────
    def run_scalper(self) -> None:
        """Cancel expired scalp rests, then fire fresh scalps for every
        recent arb fill on the queue. Each scalp posts a *limit* on the
        opposite side at fill_price ± SCALP_OFFSET so the position rotates
        back to cash within roughly one broadcast cycle when the touch
        bounces back. If it doesn't bounce in SCALP_DEADLINE_MS, we cancel
        and let the inventory taper / unwind take over."""
        now = now_ms()

        # Cancel scalp rests that overran their deadline.
        for oid in list(self.scalp_live):
            ex, tk, deadline = self.scalp_live[oid]
            if now < deadline:
                continue
            conn = self.connections.get(ex)
            if conn and conn.connected:
                conn.enqueue({
                    "type": "cancel_order",
                    "user_request_id": self.req_id(f"scalp_cxl-{tk}"),
                    "order_id": oid,
                    "instrument_id": f"{ex}-{tk}",
                })
            self.scalp_live.pop(oid, None)

        # Drain queued scalp intents.
        while self.scalp_queue:
            ex, tk, side_filled, fill_px, fill_qty, deadline = self.scalp_queue.popleft()
            if now >= deadline or fill_qty <= 0:
                continue
            if side_filled == "bid":
                # We just bought → rest a sell at fill + offset.
                opp_side = "ask"
                scalp_px = fill_px + SCALP_OFFSET_CENTS
            else:
                opp_side = "bid"
                scalp_px = max(1, fill_px - SCALP_OFFSET_CENTS)
            leg = Leg(ex, tk, opp_side, fill_qty, scalp_px,
                      order_type="limit",
                      note=f"scalp_{tk}",
                      strategy="SCALP")
            self.fire_one(leg)
    ...
    def etf_basket_arbs(self) -> list[Plan]:
        ...
        for etf in ETFS:
            ...
            for etf_v in LISTINGS[etf]:
                ...
                const_venues = tuple({const_bid[tk][0] for tk in basket})
                threshold = self._threshold("ETF_BUY", etf, BASE_ARB_EDGE,
                                            etf_v, *const_venues)
                ...
                const_ask_venues = tuple({const_ask[tk][0] for tk in basket})
                threshold = self._threshold("ETF_SELL", etf, BASE_ARB_EDGE,
                                            etf_v, *const_ask_venues)
    ...
    def sub_etf_arbs(self) -> list[Plan]:
        ...
        for super_etf, (sub_etf, complement) in SUB_ETF_LINKS.items():
            ...
            sub_a_venues = (sup_buy[0], sub_sell[0]) + tuple(
                {comp_bid[tk][0] for tk in complement}
            )
            thr = self._threshold("SUB_ETF_A", super_etf, BASE_SUB_ETF_EDGE, *sub_a_venues)
            ...
            sub_b_venues = (sup_sell[0], sub_buy[0]) + tuple(
                {comp_ask[tk][0] for tk in complement}
            )
            thr = self._threshold("SUB_ETF_B", super_etf, BASE_SUB_ETF_EDGE, *sub_b_venues)
    ...
    def cross_venue_arbs(self) -> list[Plan]:
        ...
        for tk in LISTINGS:
            ...
            threshold = self._threshold("XV", tk, BASE_XV_EDGE,
                                        best_a[0], best_b[0])
    ...
    def passive_mm_refresh(self) -> None:
        ...
        for tk in MM_INSTRUMENTS:
            for v in LISTINGS.get(tk, set()):
                ...
                # Inventory-tapered MM size. During cold-start the taper is
                # forced to 1.0 by inventory_taper() and the base size is
                # bumped — cash and headroom are abundant in the first 5s.
                base_qty = COLD_START_MM_QTY if self.is_cold_start(v) else MM_QTY
