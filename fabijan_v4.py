#!/usr/bin/env python3
"""
fabijan_v4.py — AlgoTrade 2026 trading bot, fabijan v4.

Forked from namikv2 (the previous best). Inherits everything namikv2 has:
  - per-instrument adaptive thresholds
  - depth walking on book sweeps
  - microprice in fair value
  - inventory-tapered sizing
  - per-strategy P&L instrumentation
  - cluster-aware leg preference (tie-break)
  - cross-cluster drift penalty
  - settlement-aware unwind
  - Euronext venue casing fix (server expects "Euronext", not "EURONEXT")

Adds three location-awareness fixes not in namikv2:

  1. **Per-segment cluster re-detection.** namikv2 ran auto_detect_cluster()
     once at startup and froze the result for the whole round. The team's
     physical location rotates every 10-min segment between NYSE/ZSE/HKEX,
     so freezing the cluster makes the cluster-preference and drift-penalty
     mechanisms wrong for 2 of every 3 segments. v4 re-probes on every
     welcome with a 30s debounce; only the first welcome after a segment
     boundary actually fires the probe, the rest skip.

  2. **Cluster-first cross-venue counterparty selection.** namikv2 picked the
     global best bid + global best ask, then widened the threshold by 3¢
     per cross-cluster leg. But the picked counterparty is unchanged — so
     from NYSE we'd reach for an HKEX ask 180ms away when TMX has an ask
     only 1¢ worse and is 11ms away. v4 scans nearby-cluster pairs first
     and only falls back to global when no nearby pair clears threshold.
     Fallback is disabled at the constrained seat (HKEX) entirely.

  3. **Constrained-seat strategy posture (HKEX).** When my_cluster == "ASIA":
     - ETF basket / sub-ETF arb skip plans whose legs span outside ASIA.
       (ETFA on HKEX needs NGUP/KTST/JZRO from ZSE at 150ms — net negative.)
     - Cross-venue arb stays inside the ASIA triangle (HKEX/SSE/JPX).
     - Passive MM expands to cover all HKEX-listed instruments, soaking up
       the rate-limit budget freed by skipping cross-cluster trades.
"""
...
# Per-home-cluster set of "nearby" venues — within ~50ms RTT of the home seat.
# Used for cluster-first XV scanning and to filter ETF basket-arb legs.
# Derived from the inter-exchange latency matrix in the participant guide.
NEARBY_BY_CLUSTER: dict[str, set[str]] = {
    # NA seat (NYSE): NYSE-NASDAQ 1ms, NYSE-TMX 11ms.
    "NA":   {"NYSE", "NASDAQ", "TMX"},
    # EU seat (LSE/Euronext): LSE-Euronext 6ms, both to ZSE 22-24ms.
    "EU":   {"LSE", "Euronext", "ZSE"},
    # ZSE seat: ZSE-Euronext 22ms, ZSE-LSE 24ms.
    "ZSE":  {"ZSE", "LSE", "Euronext"},
    # ASIA seat (HKEX): HKEX-SSE 19ms, HKEX-JPX 37ms, SSE-JPX 18ms.
    "ASIA": {"HKEX", "SSE", "JPX"},
    # NSE seat: NSE-HKEX/SSE/JPX 53ms; NSE itself.
    "IN":   {"NSE", "HKEX", "SSE", "JPX"},
}

# Constrained-seat clusters — at these locations we refuse far cross-cluster
# legs because the RTT (130-180ms) eats more cents of drift than any plausible
# threshold widening can compensate. ASIA is the only one in the actual
# rotation (NYSE/ZSE/HKEX); listed singly so the policy is explicit.
CONSTRAINED_CLUSTERS: set[str] = {"ASIA"}

# Per-cluster passive-MM instrument shortlist. At constrained seats we expand
# the list to cover more local-listed names since cross-cluster arb is off.
MM_DEFAULT = ["CARD", "SIMP", "ETFA", "ETFB", "GOLD", "XAG", "ETFSH"]
MM_INSTRUMENTS_BY_CLUSTER: dict[str, list[str]] = {
    "NA":   MM_DEFAULT,
    "EU":   MM_DEFAULT,
    "ZSE":  MM_DEFAULT,
    "IN":   MM_DEFAULT,
    # At HKEX — local single-name MM expansion. All listed on HKEX.
    "ASIA": MM_DEFAULT + ["OIT", "FSR", "XFR", "KOTD", "INA",
                          "JNAF", "DLKV", "MDKA", "ETFB3"],
}
...
MM_QTY             =      4
MM_REFRESH_S       =      1.5
# MM_INSTRUMENTS replaced by MM_INSTRUMENTS_BY_CLUSTER above; the active list
# is selected at runtime by Hub.passive_mm_refresh based on self.my_cluster.
...
log = logging.getLogger("fabijan_v4")
...
class Hub:
    def __init__(self, active_venues: list[str], my_cluster: str = "NA"):
        ...
        # Cluster-derived state. Re-computed whenever my_cluster changes
        # (which happens on segment boundary via maybe_redetect_cluster).
        self.nearby_venues: set[str] = set()
        self.is_constrained: bool = False
        self.mm_instruments: list[str] = []
        self._refresh_cluster_state()

        # Re-detection plumbing. The re-detect runs at most once per ~30s,
        # which is well under the 10-min segment length but well over the
        # 1s burst of welcomes that arrive when all 10 venues come back up
        # after a segment boundary.
        self._detect_lock = asyncio.Lock()
        self._last_detect_time = time.monotonic()

    def _refresh_cluster_state(self) -> None:
        """Recompute derived sets from self.my_cluster. Call after my_cluster changes."""
        self.nearby_venues = NEARBY_BY_CLUSTER.get(self.my_cluster, set())
        self.is_constrained = self.my_cluster in CONSTRAINED_CLUSTERS
        self.mm_instruments = MM_INSTRUMENTS_BY_CLUSTER.get(
            self.my_cluster, MM_DEFAULT
        )

    async def maybe_redetect_cluster(self) -> None:
        """Re-probe venue latencies and update self.my_cluster if changed.

        Debounced: returns immediately if a detection ran in the last 30s.
        That makes it safe to call from every Connection.run() after each
        welcome — only the first welcome after a segment boundary actually
        fires the probe.
        """
        now = time.monotonic()
        if now - self._last_detect_time < 30.0:
            return
        async with self._detect_lock:
            # Re-check inside the lock — a concurrent caller may have
            # finished a probe while we were waiting for the lock.
            if time.monotonic() - self._last_detect_time < 30.0:
                return
            new_cluster = await auto_detect_cluster()
            self._last_detect_time = time.monotonic()
            old_cluster = self.my_cluster
            if new_cluster != old_cluster:
                log.info("cluster rotated: %s → %s", old_cluster, new_cluster)
                self.my_cluster = new_cluster
                self._refresh_cluster_state()
            else:
    ...
    def _best_buy_venue(self, ticker: str, my_cluster: Optional[str] = None,
                        venue_filter: Optional[set[str]] = None):
        """Lowest ask across venues, breaking ties by same-cluster.

        venue_filter, if supplied, restricts the search to that set — used
        at constrained seats to forbid far cross-cluster legs entirely.
        """
        best = None  # (px, qty, venue, in_cluster_flag)
        for v in LISTINGS[ticker]:
            if v not in self.active_venues:
                continue
            if venue_filter is not None and v not in venue_filter:
                continue
    ...
    def etf_basket_arbs(self) -> list[Plan]:
        out: list[Plan] = []
        # When at constrained seat (HKEX), require ALL legs to be in the
        # nearby set — otherwise the basket scatters to ZSE or NA at
        # 130-180ms RTT and the arb fires on stale constituent prices.
        venue_filter = self.nearby_venues if self.is_constrained else None
        for etf in ETFS:
            ...
            ok = True
            for tk in basket:
                bb = self._best_sell_venue(tk, self.my_cluster, venue_filter=venue_filter)
                ba = self._best_buy_venue(tk, self.my_cluster, venue_filter=venue_filter)
                if bb is None or ba is None:
                    ok = False; break
                const_bid[tk] = bb
                const_ask[tk] = ba
            ...
            for etf_v in LISTINGS[etf]:
                if etf_v not in self.active_venues:
                    continue
                if venue_filter is not None and etf_v not in venue_filter:
                    continue
    ...
    def sub_etf_arbs(self) -> list[Plan]:
        out: list[Plan] = []
        # Same constrained-seat rule as etf_basket_arbs: at HKEX, every leg
        # (super, sub, complement) must be in the nearby set.
        venue_filter = self.nearby_venues if self.is_constrained else None
        for super_etf, (sub_etf, complement) in SUB_ETF_LINKS.items():
            ...
            if venue_filter is not None:
                sup_venues = [v for v in sup_venues if v in venue_filter]
                sub_venues = [v for v in sub_venues if v in venue_filter]
    ...
    def cross_venue_arbs(self) -> list[Plan]:
        ...
        def best_pair_in(venues_subset: list[str], tk: str):
            """Return ((sell_venue, sell_px, sell_qty), (buy_venue, buy_px, buy_qty))
            for the highest bid + lowest ask within venues_subset, or None if
            < 2 venues quote both sides."""
            best_b = best_a = None
            for v in venues_subset:
                bk = self.books[(v, tk)]
        ...
        for tk in LISTINGS:
            ...
            # 1) Cluster-first scan: prefer any nearby-set pair that meets
            # threshold WITHOUT a drift penalty. namikv2 picked the global
            # best regardless of distance and only widened the threshold;
            # that left fast in-cluster opportunities on the table whenever
            # a far venue was slightly cheaper.
            nearby_venues_for_tk = [v for v in venues if v in self.nearby_venues]
            chosen_pair = None
            chosen_edge = 0
            chosen_drift = 0
            if len(nearby_venues_for_tk) >= 2:
                local = best_pair_in(nearby_venues_for_tk, tk)
                if local is not None:
                    lb, la = local
                    edge_local = lb[1] - la[1]
                    # Record observed spread for adaptive threshold.
                    if edge_local > 0:
                        self.spread_tracker.record("XV", tk, edge_local)
                    if edge_local >= threshold:
                        chosen_pair = (lb, la)
                        chosen_edge = edge_local
                        chosen_drift = 0

            # 2) Global fallback — only if nearby scan didn't fire AND we're
            # not at a constrained seat. At HKEX (ASIA) we refuse far legs
            # entirely because the RTT (130-180ms) makes the trade
            # economically negative even with a widened threshold.
            if chosen_pair is None and not self.is_constrained:
                glob = best_pair_in(venues, tk)
                if glob is None:
                    continue
                gb, ga = glob
                top_edge = gb[1] - ga[1]
                ...
                buy_v_g, sell_v_g = ga[0], gb[0]
                drift = 0
                if CLUSTER_OF.get(buy_v_g) != self.my_cluster:
                    drift += CROSS_CLUSTER_DRIFT_CENTS
                if CLUSTER_OF.get(sell_v_g) != self.my_cluster:
                    drift += CROSS_CLUSTER_DRIFT_CENTS
                if top_edge < threshold + drift:
                    continue
                chosen_pair = (gb, ga)
                chosen_edge = top_edge
                chosen_drift = drift

            if chosen_pair is None:
                continue

            best_b, best_a = chosen_pair
            buy_v, buy_px_top, _ = best_a
            sell_v, sell_px_top, _ = best_b
            top_edge = chosen_edge
            effective_threshold = threshold + chosen_drift
    ...
    def passive_mm_refresh(self) -> None:
        now = time.monotonic()
        # self.mm_instruments is set by _refresh_cluster_state and rotates
        # with the home cluster. ASIA seats get an expanded list to soak up
        # the rate-limit budget freed by skipping cross-cluster arbs.
        for tk in self.mm_instruments:
        ...
        while not self.hub.stop:
            try:
                async with ws_connect(self.url, open_timeout=5, ping_interval=20,
                    ...
                    # Trigger per-segment cluster re-detect. After end_of_round
                    # all 10 connections reconnect simultaneously and call this;
                    # the 30s debounce inside maybe_redetect_cluster ensures only
                    # the first one to land actually probes.
...
def main() -> None:
    ...
    for v in args.venues.split(","):
        ...
        if canon not in active:
            active.append(canon)
    log.info("starting fabijan_v4 on venues: %s", active)
