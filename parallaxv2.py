#!/usr/bin/env python3
"""Rail-liquidity bot for the AlgoTrade 2026 exchange API — v2.

Same edge as parallax.py, but trades BOTH CARD and SIMP on every exchange
(both list on all 10). One WebSocket per exchange, one shared
`ExchangeState` (cash + reservations + rate-limit are all per-exchange),
but a separate `RailEdgeStrategy` per (exchange, instrument). Each
strategy plans for its own instrument; their planned orders are
dispatched in sequence within a tick so the second strategy's
`state.free_cash()` reservation calc already includes the first
strategy's just-issued bids.
...
def default_rail_configs() -> dict[str, list[RailConfig]]:
    """For every exchange, run a CARD rail AND a SIMP rail in parallel.

    Both instruments float around $100; the same low/high rail prices
    work for both. Per-venue high_ask_price tweaks didn't matter much
    in v1 — uniform here keeps the config readable.
    """
    out: dict[str, list[RailConfig]] = {}
    for ex in EXCHANGES:
        out[ex] = [
            RailConfig(ex, "CARD", 7001, 11000, 9900, 10100),
            RailConfig(ex, "SIMP", 7001, 11000, 9900, 10100),
        ]
    return out
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the AlgoTrade rail edge bot v2.")
    parser.add_argument(
        "--exchanges",
