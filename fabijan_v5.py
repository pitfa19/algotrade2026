#!/usr/bin/env python3
"""
fabijan_v5.py — AlgoTrade 2026 trading bot, fabijan v5.

Forked from namikv2. Same architecture, same strategies, same Euronext casing,
same cluster-awareness — but tuned for the SCALPER profile observed in top
competitors:

  - **80% of capital sits in cash.** Top bots never let any single position
    grow large. They take many small round-trips, each of which moves cash
    by a few thousand dollars per exchange and exits before the next signal.
    Result: high turnover, low capital deployment, near-zero directional risk.

The changes vs namikv2:

  1. **Tight inventory caps.** SOFT_POS_MAX drops from 1800 → 250. At ~$100/share
     that's ~$25k of inventory per (exchange, instrument) instead of ~$180k.
     This is the main capital-efficiency lever. Combined with namikv2's existing
     inventory taper (now starting at 30% instead of 50%), positions almost
     never hit the cap; new orders shrink long before that.

  2. **Smaller per-trade size.** ARB_MAX_K 25 → 5, XV_MAX_QTY 100 → 30,
     MM_QTY 4 → 3. Each fire takes a small bite. Big edges still get hit
     repeatedly across consecutive ticks, but no single fire dumps inventory.

  3. **Lower thresholds, more fires.** BASE_XV_EDGE 4 → 2, BASE_ARB_EDGE 4 → 2,
     BASE_SUB_ETF_EDGE 6 → 3. ADAPT_QUANTILE 0.30 → 0.20. Adaptive learning
     still floors at the base; the floors just dropped. With smaller sizes
     this is safe — we capture more edges without bloating positions.

  4. **Faster MM refresh.** MM_REFRESH_S 1.5 → 0.5. Quotes update every 500ms
     instead of every 1.5s, so we step inside the MM more often.

  5. **MM universe expanded** to every instrument we list. namikv2 only MM'd
     7 default tickers; fabijan_v5 quotes everything. With smaller MM_QTY this
     fits the rate-limit budget.

  6. **Inventory-aware MM skew.** namikv2's MM skew was based on microprice
     (signal direction). fabijan_v5 adds a position-aware overlay: when long,
     skew quotes DOWN (cheaper ask, less attractive bid) to bleed inventory;
     when short, skew UP. This is the active force keeping positions near zero
     between explicit unwinds.

Inherits unchanged from namikv2: adaptive thresholds, depth walking, microprice
fair value, settlement-aware unwind, per-strategy P&L stats, auto cluster
detection (still once at startup — fix that in v4 if v3 is the right base).
"""
...
RATE_PER_S         =    400
# v3: tight per-instrument inventory cap. Hard cap is 2000 long / -200 short.
# Top competitors keep ~80% of cash idle, which means small positions per line.
# Dropping SOFT_POS_MAX from 1800 → 250 is the main capital-efficiency lever:
# at ~$100/share, peak deployment per (exchange, instrument) goes from $180k
# to $25k. Aggregate cash stays near $100k/exchange most of the time.
SOFT_POS_MAX       =    250
SOFT_POS_MIN       =   -180
SOFT_CASH_FLOOR    = -4_500_000

# v3: lower base thresholds. With smaller per-trade sizes (below) we can
# afford to fire on edges namikv2 would skip. The adaptive layer still
# floors at these values during wide regimes.
BASE_XV_EDGE       =      2
BASE_ARB_EDGE      =      2
BASE_SUB_ETF_EDGE  =      3
...
ADAPT_QUANTILE       =     0.20   # v3: lower (was 0.30) — fire more often
ADAPT_MIN_SAMPLES    =    20      # need this many in-window samples before activating

# Inventory taper. v3: kicks in earlier (30% vs 50% of cap) and saturates
# earlier (70% vs 95%) — combined with the much lower SOFT_POS_MAX, this
# means new orders shrink as soon as we have a meaningful position, pushing
# the bot back toward flat after each round-trip.
TAPER_START_FRAC   =      0.3
TAPER_END_FRAC     =      0.7
...
# v3: smaller per-trade sizes. The same tape generates many more fires;
# each fire takes a small bite. Big edges get captured across consecutive
# ticks rather than in a single oversized fill that strands inventory.
ARB_MAX_K          =      5       # was 25
XV_MAX_QTY         =     30       # was 100
MM_INSIDE_TICK     =      1
MM_QTY             =      3       # was 4
MM_REFRESH_S       =      0.5     # was 1.5 — refresh quotes 3× more often

# Inventory-aware MM skew threshold. When |pos| exceeds this fraction of
# SOFT_POS_MAX, the MM tilts both quotes by ±1¢ to bleed inventory.
INV_SKEW_THRESHOLD_FRAC =  0.3

# v3: MM the entire universe. namikv2 quoted only 7 default tickers; v3
# quotes everything we list, soaking up noise-trader flow on instruments
# that the arb strategies barely touch.
...
log = logging.getLogger("fabijan_v5")
...
def main() -> None:
    ...
    for v in args.venues.split(","):
        ...
        if canon not in active:
            active.append(canon)
    log.info("starting fabijan_v5 on venues: %s", active)
