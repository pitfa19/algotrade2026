/*
 * AlgoTrade 2026 â€” Demo Trading Bot (C++)
 *
 * A reference trading bot that demonstrates the full exchange API.
 * The bot is structured around a pluggable Strategy interface â€” subclass
 * Strategy and implement the virtual methods with your own logic.
 *
 * Architecture:
 *     ExchangeConnection  â€” manages one WebSocket connection to one exchange
 *     MarketState         â€” parsed, queryable snapshot of the latest market data
 *     Strategy            â€” abstract base class with virtual callbacks
 *     SimpleStrategy      â€” minimal example that prints top-of-book
 *     Bot                 â€” orchestrates connections and dispatches to strategy
 *
 * Dependencies:
...
// Production exchange network â€” each exchange has a fixed IP, all on port 9001.
 ...
 *
 * Lifecycle:
 *     1. on_connected(bot, exchange)           â€” WebSocket connected
 *     2. on_market_data(bot, exchange, state)   â€” every 100ms per exchange
 *     3. on_fill(bot, fill)                     â€” your order was filled
 *     4. on_round_end(bot, exchange)            â€” round ended
 */
class Strategy {
public:
...
/**
 * Minimal example strategy â€” logs top-of-book data.
 *
...
public:
    ...
    bool connect() {
        try {
            ...
            // WebSocket handshake â€” production network authenticates by team IP.
...
public:
    ...
    // â”€â”€ Public API for strategies â”€â”€
