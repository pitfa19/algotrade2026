...
If `ssh` is not available, install **OpenSSH Client** from _Settings → Apps → Optional Features_ , or use [PuTTY](<https://www.putty.org>) (Host: `vm.algotrade.hr`, Port: 22).
...
For drag-and-drop, [WinSCP](<https://winscp.net>) is the easiest option — connect with protocol _SFTP_ , host `vm.algotrade.hr`, user `root`.
