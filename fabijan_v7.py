#!/usr/bin/env python3
"""
fabijan_v7.py — AlgoTrade 2026 trading bot, fabijan v7.

Forked from fabijan_v6. v6 was too cautious — it correctly identified huge
edges (CARD divergences of 30-100c, OIT 89c, ETF basket arbs of 30c+) but
sent qty=1-10 shares per fire, capturing pennies on opportunities worth
dollars per share. v7 adds **edge-proportional sizing**: marginal edges
stay small (capital-efficient, scalper profile preserved) but big edges
get scaled up so we actually capture them in 1-2 fires instead of 30.
...
# v7: position cap raised so big-edge fires have room to scale. v6's 100-cap
# blocked further fires after just 8 ETF basket k=2 fires (16 shares per
# constituent), wasting persistent edges. v7's 400 keeps the scalper profile
# (still 5× tighter than namikv2) but lets a single huge-edge fire ride.
# Combined with edge-proportional sizing (below), small edges still send
# small qty; only big ones reach into this expanded room.
SOFT_POS_MAX       =    400       # was 100 in v6, 250 in v5, 1800 in namikv2
...
# v7: per-fire size is now EDGE-PROPORTIONAL. The constants below are the
# BASE size (used for edges at threshold). Real qty for a fire is:
#     qty = base * edge_multiplier(edge_cents, threshold_cents)
# multiplier = 1× when edge ≈ threshold, scaling up to 20× for huge edges.
# So a 1-cent edge (just clearing the floor) sends qty=2 ETF / qty=10 stock
# (v6 behavior). A 50-cent edge sends qty=20 ETF / qty=100 stock — actually
# capturing the opportunity in one fire rather than 30.
ARB_MAX_K          =      2       # base (edge ≈ threshold)
ARB_MAX_K_HARD     =     30       # ceiling — even huge edges don't exceed this
XV_MAX_QTY         =     10       # base
XV_MAX_QTY_HARD    =    100       # ceiling
MM_INSIDE_TICK     =      1
...
log = logging.getLogger("fabijan_v7")
...
def edge_size_multiplier(edge_cents: float, threshold_cents: float) -> float:
    """Edge-proportional sizing. Returns a multiplier in [1.0, 20.0].

    Shape: sqrt(edge / threshold). So:
      edge ≈ threshold (just clearing floor)  → 1×    (small bite)
      edge =  5× threshold                    → 2.2×
      edge = 25× threshold                    → 5×
      edge = 50× threshold                    → 7.1×
      edge =100× threshold                    → 10×
      edge ≥400× threshold                    → 20× (capped)

    The sqrt shape lets us aggressively scale into the genuine outliers
    (CARD/MDKA/KRAS divergences of 1000c+) without exploding sizes on
    every minor edge that clears the 1c floor.
    """
    if threshold_cents < 1:
        threshold_cents = 1
    ratio = edge_cents / threshold_cents
    if ratio <= 1:
        return 1.0
    return max(1.0, min(20.0, math.sqrt(ratio)))
...
class Hub:
        ...
        for etf in ETFS:
            ...
            for etf_v in LISTINGS[etf]:
                ...
                if edge_a >= threshold:
                    ...
                    # v7: edge-proportional k cap. Big edges scale up to
                    # ARB_MAX_K_HARD = 30; small edges stay at base = 2.
                    edge_mult = edge_size_multiplier(edge_a, threshold)
                    k_cap = min(ARB_MAX_K_HARD, int(ARB_MAX_K * edge_mult))
                    k = min(k, cash_room, k_cap)
                    k = max(0, int(k * taper))
                    if k > 0:
                ...
                if edge_b >= threshold:
                    ...
                    for v, per_k_cost in venue_buy_cost.items():
                        ...
                        k = min(k, room)
                    # v7: edge-proportional k cap (matches direction A).
                    edge_mult = edge_size_multiplier(edge_b, threshold)
                    k_cap = min(ARB_MAX_K_HARD, int(ARB_MAX_K * edge_mult))
                    k = min(k, k_cap)
                    k = max(0, int(k * taper))
                    if k > 0:
    ...
    def sub_etf_arbs(self) -> list[Plan]:
        ...
        for super_etf, (sub_etf, complement) in SUB_ETF_LINKS.items():
            ...
            if edge_a_per_share >= thr:
                ...
                # v7: edge-proportional cap.
                edge_mult = edge_size_multiplier(edge_a_per_share, thr)
                k_cap = min(ARB_MAX_K_HARD, int(ARB_MAX_K * edge_mult))
                k = min(
            ...
            if edge_b_per_share >= thr:
                ...
                # v7: edge-proportional cap.
                edge_mult = edge_size_multiplier(edge_b_per_share, thr)
                k_cap = min(ARB_MAX_K_HARD, int(ARB_MAX_K * edge_mult))
    ...
    def cross_venue_arbs(self) -> list[Plan]:
        ...
        for tk in LISTINGS:
            ...
            # v7: edge-proportional cap. Big edges (CARD divergences ~30-100c
            # vs threshold ~1c) scale up to XV_MAX_QTY_HARD = 100 sh; tiny
            # edges stay at the v6 base of XV_MAX_QTY = 10 sh.
            edge_mult = edge_size_multiplier(walked_edge, effective_threshold)
            xv_cap = min(XV_MAX_QTY_HARD, int(XV_MAX_QTY * edge_mult))
            qty = min(buy_qty, sell_qty, xv_cap)
...
def main() -> None:
    ...
    for v in args.venues.split(","):
        ...
        if canon not in active:
            active.append(canon)
    log.info("starting fabijan_v7 on venues: %s", active)
